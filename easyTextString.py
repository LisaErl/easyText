...
def executeObject(obj):
    debug = False
    if debug:  print("easyTextString executeObject Start")
    if obj.TextShape == "Wires":
        shapes, obj.Font, simplified = etf.makeWiresTextString(obj.Text, obj.Font, obj.FontExt, obj.Height, obj.Width)
    elif obj.TextShape == "Faces":
        shapes, obj.Font = etf.makeFacesTextString(obj.Text, obj.Font, obj.FontExt, obj.Height, obj.Width, obj.TextShapeFusion)
    elif obj.TextShape == "Extrusion":
        shapes, obj.Font = etf.makeExtrudesTextString(obj.Text, obj.Font, obj.FontExt, obj.TextShapeExtrusion, obj.Height, obj.Width, obj.TextShapeFusion)        
    if shapes:
        obj.Shape = Part.Compound(shapes)
    else:
        obj.Shape = Part.Shape()
        obj.ViewObject.hide()
    if debug:  print("easyTextString executeObject Ende")

def initPropertiesObject(obj):
    debug = False
    if debug: print("easyTextString initPropertiesObject Start")
    try:
    ...
    if debug: print("easyTextString initPropertiesObject Ende")
    
class easyTextStringFeature:
    def __init__(self, obj):
        debug = False
        if debug: print("easyTextStringFeature __init__ Start")
        # Data Properties
        ...
        obj.Proxy = self
        self.obj = obj        
        self.Type = "easyTextString"
        if debug: print("easyTextStringFeature __init__ Ende")
    ...
    def onChanged(self, obj, prop):
        ...
        debug = False
        if debug: print("easyTextStringFeature onChanged Start")
        ...
        if prop == "TextShape":
            ...
            if hasattr(obj, 'TextShapeFusion'):
                if obj.TextShape == "Wires":
                    ...
                    obj.setPropertyStatus("TextShapeFusion", '-Hidden')
        if debug: print("easyTextStringFeature onChanged Ende")
        #redrawTextstring(obj)
        
    def editProperty(self, prop):
        debug = False
        if debug: print("easyTextStringFeature editProperty Start")
        if prop == 'Font':
            ...
            form = dlgFormat.dlgFormat(qfont, self.obj.Text, "easyTextString")
            form.exec_()
            if form.result == "OK":
...
class easyTextStringViewProvider:
    ...
    def getIcon(self):
        ...
        return os.path.join(etf.ICONPATH, 'etStringIcon.svg')
    ...
    def setEdit(self, viewObject, mode):
        #self.panel = easyTextStringTaskPanel(self.Object)
        #Gui.Control.showDialog(self.panel)
        #return True
        if mode == 0:
            FreeCADGui.Control.showDialog(easyTextStringTaskPanel(viewObject))
            return True
...
class easyTextStringWidget(QtGui.QWidget):
    
    def __init__(self, obj, *args):
        debug = False
        if debug:  print("easyTextStringWidget __init__ Start")
        ...
        if debug:  print("easyTextStringWidget __init__ Ende")
    ...
    def __UI_Font__(self):
        debug = False
        if debug:  print("easyTextStringWidget __UI_Font__ Start")
        if debug:  print("self.obj.Font: " + str(self.obj.Font))
        hbox = QtGui.QHBoxLayout()
        ...
        hbox.addStretch()
        if debug:  print("easyTextStringWidget __UI_Font__ Ende")
        return hbox 
        
    def onFontComboChanged(self):
        debug = False
        if debug:  print("easyTextStringWidget onFontComboChanged Start")
        if not self.form:
            self.qfont = QtGui.QFont()            
        self.qfont.setFamily(self.cbFamily.currentFont().family())
        #self.qfont.setPointSize(36)
        self.form = None
        if debug:  print("easyTextStringWidget onFontComboChanged Ende")
        
    def onAdvancedSettings(self):
        debug = False
        if debug: print("easyTextStringWidget onAdvancedSettings Start")
        self.form = dlgFormat.dlgFormat(self.qfont, self.text.text(), "easyTextString")
        self.form.exec_()
        if self.form.result == "OK":
            self.qfont = self.form.qfont
            self.text.setText(self.form.getText())
            self.cbFamily.setCurrentFont(self.qfont)
        if debug: print("easyTextStringWidget onAdvancedSettings Ende")
        
    def __UI_Dim__(self):
        debug = False
        if debug:  print("easyTextStringWidget __UI_Dim__ Start")
        ...
        hbox.addStretch()
        if debug:  print("easyTextStringWidget __UI_Dim__ Ende")
        return hbox 
        
    def __UI_Shape__(self):
        debug = False
        if debug:  print("easyTextStringWidget __UI_Shape__ Start")
        ...
        hbox.addStretch()
        if debug:  print("easyTextStringWidget __UI_Shape__ Ende")
        return hbox 
        
    def onShapeChanged(self):
        debug = False
        if debug:  print("easyTextStringWidget onShapeChanged Start")
        ...
        if self.sender().currentText() != "Wires":
            ...
            self.groupFuseFaces.hide()
        if debug:  print("easyTextStringWidget onShapeChanged Ende")
    
        
class easyTextStringTaskPanel:
    def __init__(self, viewObject):
        debug = False
        if debug: print("easyTextStringTaskPanel __init__ Start")
        self.viewObject = viewObject
        self.form = easyTextStringWidget(self.viewObject.Object)
        if debug: print("easyTextStringTaskPanel __init__ Ende")

    def accept(self):
        debug = False
        if debug: print("easyTextStringTaskPanel accept Start")
        ...
        if debug: print("obj.Width: " + str(obj.Width))
        obj.TextShape = self.form.cbShape.currentText()
        if debug: print("obj.TextShape: " + str(obj.TextShape))
        obj.TextShapeExtrusion = float(self.form.leExtHeight.text())
        if debug: print("obj.TextShapeExtrusion: " + str(obj.TextShapeExtrusion))
        ...
        redrawTextstring(obj)
        if debug: print("after redraw")
        document = self.viewObject.Document.Document
        document.commitTransaction()
        if debug: print("after commit")
        document.recompute()
        if debug: print("after recompute")
        FreeCADGui.ActiveDocument.resetEdit()
        if debug: print("after resetEdit")
        if debug: print("easyTextStringTaskPanel accept Ende")
        return True
    ...
    def reject(self):
        ...
        document.recompute()
        FreeCADGui.ActiveDocument.resetEdit()
        #print("easyTextStringTaskPanel reject Ende")
        return True
...
def redrawTextstring(obj):
    debug = False
    if debug: print("easyTextString redrawTextstring Start")
    if obj.Text != "":
        executeObject(obj)
    if debug: print("obj.Font: " + str(obj.Font))    
    if debug: print("obj.Text: " + str(obj.Text))
    if debug: print("easyTextString redrawTextstring Ende")
    

    
        
class CommandEasyTextString():
    """Command for creating easyTextString"""    
                
    def GetResources(self):
        return {'Pixmap'  : os.path.join(etf.ICONPATH, 'etStringIcon.svg'),
                'Accel' : "", # a default shortcut (optional)
                'MenuText': "easyTextString",
                'ToolTip' : __doc__ }
    ...
    def Activated(self):
        ...
        vp = easyTextStringViewProvider(obj.ViewObject)
        FreeCAD.ActiveDocument.recompute()
        vp.startDefaultEditMode(obj.ViewObject)


FreeCADGui.addCommand('easyTextString', CommandEasyTextString())
